...
import { MdCalendarMonth, MdAccessTime, MdLocationOn, MdSchool, MdSchedule } from "react-icons/md";
import courseData from "../../../variables/courseData";
import { fetchBookingsForStudent } from "../../../services/api";

// Helper: build today's events from enrolled courses for a given date
...
// Helper: Filter bookings by date and merge with class events
function buildBookingEvents(bookings, dateStr, studentEnrolledCourses) {
  const selectedDate = dateStr;
  let bookingEvents = [];

  bookings.forEach(booking => {
    // Get the date from either allocatedSlot (approved) or priorities (pending)
    const bookingDate = booking.allocatedSlot?.date || (booking.priorities && booking.priorities.length > 0 ? booking.priorities[0].date : null);
    const startTime = booking.allocatedSlot?.startTime || (booking.priorities && booking.priorities.length > 0 ? booking.priorities[0].startTime : "");
    const endTime = booking.allocatedSlot?.endTime || (booking.priorities && booking.priorities.length > 0 ? booking.priorities[0].endTime : "");
    const venueName = booking.allocatedSlot?.venueName || (booking.priorities && booking.priorities.length > 0 ? booking.priorities[0].venueName : "TBD");

    if (bookingDate === selectedDate) {
      // Determine if this booking is for a course or department
      ...
      bookingEvents.push({
        ...
        startTime: startTime,
        status: booking.status === 'Pending' || booking.tracker?.ar !== 'approved' ? "Pending" : "Approved",
        type: "booking",
        bookingData: booking,
...
export default function StudentSchedule() {
  ...
  // Core function to read from localStorage and rebuild schedule
  ...
  // Re-run whenever date changes
  useEffect(() => {
    refreshSchedule(selectedDate);
  }, [selectedDate, refreshSchedule]);

  // Also re-run when AccountSettingsModal fires the coursesUpdated event
  useEffect(() => {
            ...
            <div className="relative pl-4 md:pl-8">
              ...
              {todayEvents.length === 0 ? (
                ...
                todayEvents.map((event) => {
                  ...
                  if (isBooking) {
                    if (isPending) {
                      // Pending booking - orange/amber color
                  ...
                  return (
                    <div key={event.id} className="relative flex items-start gap-6 mb-8 group">
                      ...
                      {/* Event Card */}
                      <div className={`w-full rounded-[16px] ${bgColor} backdrop-blur-md p-5 shadow-sm border-y border-r border-gray-100 ${borderColor} border-l-4 transition-all duration-300 hover:-translate-y-1 ${shadowColor} dark:border-navy-700`}>
                        ...
                        {/* Source Information for Bookings */}
